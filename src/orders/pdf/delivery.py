...
def draw_header(canvas):
    ...
    canvas.drawString(1 * cm, -1.5 * cm, '納品書')
...
def draw_address(canvas, item):
    ...
    business_details = (
        ...
        'support@canal.ink',
        '',
        '納品書番号：{}'.format(item.ref_code),
        # TODO: 納品日に設定
        '納品日　　：{}'.format(item.delivery.delivery_date.strftime("%Y/%m/%d") if item.delivery.delivery_date else "---"),
...
def draw_pdf(buffer, item):
    ...
    canvas.setFont('GenShinGothic', 12)
    textobject = canvas.beginText(1 * cm, -4.5 * cm)
    textobject.textLine(item.delivery.get_full_name() + ' 様')
    canvas.drawText(textobject)
    canvas.setFont('GenShinGothic', 10, leading=14)
    textobject = canvas.beginText(1 * cm, -5.2 * cm)
    textobject.textLine(item.order.billing_address.get_full_address())
    textobject.textLine('〒' + item.order.billing_address.postal_code)
    canvas.drawText(textobject)

    canvas.setFont('GenShinGothic', 7, leading=12)
    textobject = canvas.beginText(1 * cm, -7 * cm)
    textobject.textLine("この度はcanalをご利用いただき誠にありがとうございます。")
    textobject.textLine("下記の通り納品いたします。")
    canvas.drawText(textobject)

    canvas.setFont('GenShinGothicBold', 16)
    textobject = canvas.beginText(4 * cm, -8.5 * cm)
    textobject.textLine("¥ {:,}".format(item.total))
    canvas.drawText(textobject)
    canvas.setStrokeColorRGB(0.2, 0.2, 0.2)
    canvas.setLineWidth(1)
    canvas.line(1 * cm, -9 * cm, 10 * cm, -9 * cm)
