...
def draw_header(canvas):
    ...
    canvas.drawString(1 * cm, -1.5 * cm, '請求書')
...
def draw_address(canvas, item):
    ...
    business_details = (
        ...
        'support@canal.ink',
        '',
        '請求書番号：{}'.format(item.ref_code),
        '請求日　　：{}'.format(item.created_at.strftime("%Y/%m/%d")),
...
def draw_pdf(buffer, item):
    ...
    canvas.setFont('GenShinGothicBold', 14)
    textobject = canvas.beginText(1.5 * cm, -8 * cm)
    textobject.textLine("ご請求額  ¥ {:,}".format(item.total))
    canvas.drawText(textobject)
    canvas.setStrokeColorRGB(0.2, 0.2, 0.2)
    canvas.setLineWidth(1)
    canvas.line(1 * cm, -8.5 * cm, 10 * cm, -8.5 * cm)
