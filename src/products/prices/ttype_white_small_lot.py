...
def get_unit_price(vals, extra):
    # vals
    height = Decimal(vals['size']['height'])  # 高さ
    width = Decimal(vals['size']['width'])  # 幅
    depth = Decimal(vals['size']['depth'])  # 奥行き(縦)
    quantity = Decimal(vals['quantity'])  # 注文数
    print_area_num = Decimal(vals['print_area_num'])
    design_num = vals['design_num']  # 表面生地
...
def get_mold_price(vals):
    """木型代
    """
    return Decimal(0)


def get_shipping_price(vals):
    return Decimal(0)
