...
def get_unit_price(options, extra):
    ...
    # listed price params
    unit_price = [Decimal(_) for _ in [0.28]]
    ...
    # s: 製袋
    s_0 = Decimal(0.00275) * height  # 製袋単価
    s_t = Decimal(0.5)  # 製袋単価の閾値
    s_s = Decimal(0.6)
    if t_1 <= 1:
        s_1 = max(s_s + s_0, s_s + 1)
    else:
        s_1 = max(s_s + s_0, s_t + s_s)

    # ロス率
    loss = [Decimal(_) for _ in [0.005, 0.01, 0.02]]
